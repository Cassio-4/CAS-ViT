...
class SpatialOperation(nn.Module):
    def __init__(self, dim):
        ...
        # Skip mul for quantization
        self.skip_mul = nn.quantized.FloatFunctional()

    def forward(self, x):
        return self.skip_mul.mul(x, self.block(x))
...
class AdditiveTokenMixer(nn.Module):
    ...
    def __init__(self, dim=512, attn_bias=False, proj_drop=0.):
        ...
        # Replace torch.add and torch.mul with FloatFunctional
        self.skip_ops = nn.quantized.FloatFunctional()

    def forward(self, x):
        q, k, v = self.qkv(x).chunk(3, dim=1)
        q = self.oper_q(q)
        k = self.oper_k(k)
        ### Fixing for quantization
        sum_q_k = self.skip_ops.add(q, k)
        out = self.dwc(sum_q_k)
        out = self.proj(self.skip_ops.mul(v, out))
